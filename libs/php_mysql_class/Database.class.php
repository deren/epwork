...
function query_update($table, $data, $where='1') {
	...
	foreach($data as $key=>$val) {
		...
		elseif(strcasecmp($val, 'CURRENT_TIMESTAMP')==0) $v.="CURRENT_TIMESTAMP, ";
...
#-#############################################
# desc: returns the number of query
# param: (MySQL query) the query to run on server
# returns: number of results
function query_count($table, $where='1') {
	$sql = "SELECT COUNT(*) FROM `$table` WHERE $where";
	$record = $this->query_first($sql);
	$num = intval($record["COUNT(*)"]);
	if($num==0 && $record["COUNT(*)"]!=="0")
	{
		$num=-1;
	}
	return $num;
...
#-#############################################
# desc: returns the number of results
# param: (MySQL query) the query to run on server
# returns: number of results
...
// Deren Add
function prefix($db_name)
{
	return $this->pre.$db_name;
}
function GetPreviousInsertID()
{
	return mysql_insert_id($this->link_id);
}

var $DoDebug = null; // Prototype : DebugFunc($msg)
// For callback debug function
